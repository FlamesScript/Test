...
local Gui = game:GetService("CoreGui"):FindFirstChild("SimpleLib")
...
local function createOptionHolder(holderTitle, parent, parentTable, subHolder)
	...
	if not subHolder then
        ...
        -- Credits To Tora IsMe For This Draggable Thing
...
local function createSlider(option, parent)
	...
	local inContact
	main.InputBegan:Connect(function(input)
		if not sliding and input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
...
local function createColorPickerWindow(option)
    ...
    option.hue.InputBegan:Connect(function(Input)
        if not editinghue and Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
            dragging = true
            editinghue = true
    ...
    option.hue.InputEnded:Connect(function(Input)
        if editinghue and Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
            dragging = false
            editinghue = false
        end
    end)
    ...
    option.satval.InputBegan:Connect(function(Input)
        if not editingsatval and Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
            dragging = true
            editingsatval = true
        end
    end)
    ...
    option.satval.InputEnded:Connect(function(Input)
        if editingsatval and Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
            dragging = false
            editingsatval = false
        end
    end)
...
-- I don't know what is this things but Credits to Tora IsMe
inputService.InputBegan:Connect(function(input)
	if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
		if library.activePopup then
